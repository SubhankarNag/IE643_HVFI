...
class Model:
    def __init__(self, local_rank=-1, arbitrary=False):
        ...
        # added
        self.l_per = VGGPerceptualLoss()
        self.l_per.to(device)
    ...
    def load_model(self, path, rank=0):
        ...
        if rank <= 0:
            self.flownet.load_state_dict(
                convert(torch.load('{}'.format(path), map_location=device)))
    ...
    def update(self, imgs, gt, learning_rate=0, mul=1, training=True, flow_gt=None):
        ...
        # perceptual loss
        l_per = 0.5 * self.l_per(merged[2], gt)
        ...
        if training:
            ...
            loss_G = loss_l1 + loss_tea + loss_distill * 0.01 + l_per
            loss_G.backward()
            self.optimG.step()
        else:
            flow_teacher = flow[2]
        return merged[2], {
            ...
            'loss_per':l_per,
            'loss_all':(loss_l1 + loss_tea + l_per).detach().cpu().numpy()
        }
